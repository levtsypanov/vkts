...
describe(getMouseClassName, () => {
  ...
  describe('with `styles` argument', () => {
    ...
    it("returns 'none' className if there is no mouse", () =>
      expect(getMouseClassName('base', undefined, styles)).toBe(styles['base--mouse-none']));

    it("returns className with 'has' if mouse is true", () =>
      expect(getMouseClassName('base', true, styles)).toBe(styles['base--mouse-has']));

    it("returns className with 'has-not' if mouse is false", () =>
      expect(getMouseClassName('base', false, styles)).toBe(styles['base--mouse-has-not']));
  });
});